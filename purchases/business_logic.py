    ...
    def import_purchases(data):
        ...
        for index, row in data.iterrows():
            ...
            try:
                ...
                purchase.style = '' if pd.isna(row['style']) else str(row['style'])
                purchase.size = '' if pd.isna(row['size']) else str(row['size'])
                purchase.color = '' if pd.isna(row['color']) else str(row['color'])
                purchase.company = '' if pd.isna(row['company']) else str(row['company'])
                purchase.warehouse = '' if pd.isna(row['warehouse']) else str(row['warehouse'])
